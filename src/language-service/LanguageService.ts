import { Sql_scriptContext, ExpressionContext, From_clauseContext, Select_statementContext, Selected_listContext } from "../ANTLR/PlSqlParser";
...
export default class TodoLangLanguageService {
  validate(code: string): ITodoLangError[] {
    ...
    const ast: Sql_scriptContext = parseAndGetASTRoot(code);
...
// 这里不应是any
function checkSemanticRules(ast: any): ITodoLangError[] {
      ...
      if (node instanceof Selected_listContext) {
        ...
        if (diffColumnData.length > 0) {
          errors.push({
            ...
            startColumn: node.start.start + 3,
            startLineNumber: node.stop.line
          });
        }
        ...
        if (diffData.length > 0 && text.startsWith(FROM)) {
          errors.push({
            ...
            startColumn: node.start.start + 6,
            startLineNumber: node.stop.line
          });
        }
        if (repetTables.length) {
          errors.push({
            code: "3",
            endColumn: node.stop.stop,
            endLineNumber: node.stop.line,
            message: `table ${repetTables} is repeat`,
            startColumn: node.start.start,
            startLineNumber: node.stop.line
          });
        }
      }
...
//ExpressionContext
function splitToGetTable(node: any) {
  const splitRes = [];
  const tableRes = [];
  const columnRes = [];
